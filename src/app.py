...
from panda3d.core import Point3, Point2, ClockObject
...
class App(ShowBase):
    ...
    def __init__(self,
              ...
              renderer: callable,
              show_fps: bool=False,
              show_cam_info: bool=False,
              show_window_size: bool=False,
        ...
        self.show_cam_info = show_cam_info
        self.show_window_size = show_window_size
        self.setup_debug_labels()
        
        self.show_world_origin = show_world_origin
        if self.show_world_origin:
    ...
    def setup_debug_labels(self):
        ...
        if self.show_window_size:
            self.debug_label_3 = label(text="win_size", pos=vec2(0, 0.8), size=0.05, parent=self.a2dLeftCenter)
        if self.show_fps:
            self.debug_label_4 = label(text="fps", pos=vec2(0, 0.75), size=0.05, parent=self.a2dLeftCenter)
    ...
    def update_debug_labels(self):
        ...
        if self.show_fps:
            self.debug_label_4.text = f"FPS: {ClockObject.getGlobalClock().getAverageFrameRate():.0f}"
